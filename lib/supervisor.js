...
function run (args) {
  ...
  if(watch) findAllFiles(watch, watchFile);
};
...
function watchFile (watch) {
  sys.debug("Watching: " + watch);
  fs.watchFile(watch, crash);
}

var findAllFiles = function(path, callback) {
        ...
        fs.stat(path + '/' + fileName, function(err, stats) {
          if(err) {
            ...
            if(stats.isDirectory()) {
              findAllFiles(path + '/' + fileName, callback);
            }
            else {
              if(fileName.match(/.*\.js$/)) {
                sys.debug('Found file: ' + path + '/' + fileName);
