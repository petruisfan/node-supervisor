...
function run (args) {
  ...
  while (arg = args.shift()) {
    ...
    } else if (arg === "--browser-refresher" || arg === "-b")  {
      browserRefresher = true
    } else if (arg === "--") {
      program = args;
      break;
  ...
  //starts the socket.io server if the browserRefresher is enabled
  if (browserRefresher) {
    var io = socketIO.listen(7272);
    io.set("log level", 2);
...
function startProgram (prog, exec) {
  ...
  // If the browserRefresher is true wait a 10th of a second and refresh the browser
  util.debug(browserRefresher && socket);
  if (browserRefresher && socket) {
    setTimeout(function() {
      socket.emit("reload")
    }, 100);
  }
