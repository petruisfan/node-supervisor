...
function run (args) {
  var arg, dummyFunction, watch, ignore, program, extensions, executor, poll_interval, debugFlag, debugBrkFlag;
  var nodejsFlags = [];
  ...
  while (arg) {
    if (arg === "--help" || arg === "-h" || arg === "-?") {
      ...
      debug = false;
      util.debug = dummyFunction;
      util.puts = dummyFunction;
      ...
      poll_interval = parseInt(args.shift(), 10);
      ...
      nodejsFlags.unshift(args.shift());
    } else if (arg === "--") {
      program = args;
      break;
  ...
  var addNodeFlag = function(flag) {
    program.unshift(flag);
    if (executor === "coffee") {
      // coffee needs to pass node flags with --nodejs
  ...
  if (debugBrkFlag) {
    addNodeFlag("--debug-brk");
  }

  nodejsFlags.forEach(function(nodejsFlag) {
    addNodeFlag(nodejsFlag);
  });
...
function help () {
  print
    ...
    ("  supervisor --nodejs --harmony myapp.coffee")
    ("");
}
...
function startProgram (prog, exec) {
  ...
  if (child.stdout) {
    ...
    child.stdout.addListener("data", function (chunk) { if (chunk) util.print(chunk); });
    child.stderr.addListener("data", function (chunk) { if (chunk) util.debug(chunk); });
  }
...
var isWindowsWithoutWatchFile = process.platform === 'win32' && parseInt(nodeVersion[1], 10) <= 6;
