...
// function print (m, n) { console.log(m+(!n?"\n":"")); return print; }
    ...
    function startProgram(prog, exec) {
        ...
        crash_queued = false;
        child = spawn(exec, prog, {stdio: 'inherit'});
    ...
    function watchGivenFile(watch, poll_interval) {
        if (isWindowsWithoutWatchFile || forceWatchFlag) {
            watchers.push(fs.watch(watch, {persistent: true, interval: poll_interval}, crashWin));
        } else {
            watchers.push(fs.watchFile(watch, {persistent: true, interval: poll_interval}, function (oldStat, newStat) {
