...
var changedFileArg = null;

// crash is triggered by 'exit' event, so no easy way to pass the file that caused it.
// since supervisor is only watching one file anyway, store this globally.
var crashCausedByFile = null;
...
function help () {
  print
    ...
    ("")
    ("  --changed-file-arg")
    ("    Passes the <program> a command-line parameter of this name, with the file that changed.")
    ("    Assumes an '--arg=val' format (e.g. 'file' is passed as '--file=<filename>).")
    ("")
...
function watchGivenFile (watch, poll_interval) {
  if (isWindowsWithoutWatchFile)
    ...
    fs.watchFile(watch, { persistent: true, interval: poll_interval }, function(){
      crashCausedByFile = watch;
      crashOther.apply(null, arguments);
    });
  if (verbose)
