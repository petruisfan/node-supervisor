...
function run (args) {
  ...
  while (arg = args.shift()) {
    if (arg === "--help" || arg === "-h" || arg === "-?") {
      ...
      forceWatchFlag = true;
    } else if (arg === "--changed-file-arg") {
      changedFileArg = args.shift();
    } else if (arg === "--") {
      program = args;
      break;
  ...
  util.puts("")
  util.debug("Running node-supervisor (modified) with");
...
function help () {
  print
    ("")
    ("Node Supervisor (modified) is used to restart programs when they crash.")
    ...
    ("")
    ("  --changed-file-arg")
    ("    The name of the parameter that will be passed to the supervised program.")
    ("    The value will be a comma-delimited list of files that changed.")
    ("")
    ("  -h|--help|-?")
...
function startProgram (prog, exec) {
  ...
  var progCopy = prog.slice(0);
  if (changedFileArg) {
    progCopy.push(changedFileArg);
    progCopy.push(filesChanged.join(','));
  }

  util.debug("Starting child process with '" + exec + " " + progCopy.join(" ") + "'");
  crash_queued = false;
  var child = exports.child = spawn(exec, progCopy, {stdio: 'inherit'});
  if (child.stdout) {
...
function watchGivenFile (watch, poll_interval) {
  if (isWindowsWithoutWatchFile || forceWatchFlag)
    ...
    fs.watchFile(watch, { persistent: true, interval: poll_interval }, function(oldStat, newStat) {
      filesChanged.push(watch);
      crashOther(oldStat, newStat);
    });
  if (verbose)
