...
function run (args) {
  ...
  if (ignore) {
    ...
    ignoreItems.forEach(function (ignoreItem) {
      if (!ignoreItem.match(/^\/.*/)) { // watch is not an absolute path
        // convert watch item to absolute path
        ignoreItem = process.cwd() + '/' + ignoreItem;
      }
      ignorePaths[ignoreItem] = true;
...
var findAllWatchFiles = function(path, ignoredPaths, callback) {
        ...
        fs.readdir(path, function(err, fileNames) {
          if(err) {
            ...
            fileNames.forEach(function (fileName) {
              findAllWatchFiles(path + '/' + fileName, ignoredPaths, callback);
            });
          }
        });
      } else {
